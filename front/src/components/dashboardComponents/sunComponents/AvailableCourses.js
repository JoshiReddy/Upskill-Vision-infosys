...
const AvailableCourses = ({ updateTrigger, dashboardData, name }) => {
  ...
  const handleFetchCourses = async () => {
    try {
      const response = await fetch(
        "http://localhost:8080/api/admin_course/fetch-courses",
        {
          method: "GET",
          headers: {
  ...
  const handleDeleteCourse = async (courseId) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8080/api/admin_course/delete-course/${courseId}`,
        {
          method: "DELETE",
          headers: {
            "Content-type": "application/json",
  ...
  const handleUpdateCourse = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/admin_course/update-course/${editingCourse[0]}`,
        {
          method: "PUT",
          headers: {
            "Content-type": "application/json",
      ...
      <>
        ...
        {data ? (
          <div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
            ...
            <h2>Quiz Progress</h2>
            {(!data || data.length === 0) && <p>No Users enrolled</p>}
            ...
            >
              <thead>
                <tr style={{ backgroundColor: "#f4f4f4" }}>
                  ...
                  <th style={tableCellStyle}>Module Title</th>
                  <th style={tableCellStyle}>Module Completion Status</th>
                  <th style={tableCellStyle}>Progress (%)</th>
                  <th style={tableCellStyle}>Quiz Title</th>
                  <th style={tableCellStyle}>Quiz Completion Status</th>
                  <th style={tableCellStyle}>Score</th>
                  <th style={tableCellStyle}>Total Score</th>
                </tr>
              </thead>
              <tbody>
                {data.map((row, index) => (
  ...
  const handleFetchEnrolledStudents = async (courseId) => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/admin_course/enrolled-students/${courseId}`,
        {
          method: "GET",
          headers: {
            "Content-type": "application/json",
    ...
    <div className="flex flex-col">
      ...
      {selectedCourse && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 overflow-auto">
          <div className="max-h-screen w-full max-w-4xl bg-white p-6 rounded-lg shadow-lg overflow-y-auto">
            ...
            <div className="flex flex-row flex-wrap justify-around">
              ...
              {data && <QuizProgressTable data={data} />}
            </div>
          </div>
        </div>
      )}
