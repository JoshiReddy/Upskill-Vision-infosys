...
const Login = () => {
    ...
    if (formValidation()) {
      try {
        ...
        const response = await fetch(
          "http://localhost:8080/api/authentication/login-verification",
          {
            method: "POST",
            headers: {
  ...
  const handleOTP = async (e) => {
    ...
    if (otpErrors.length === 0) {
      ...
      try {
        const response = await fetch(
          "http://localhost:8080/api/authentication/verify-otp",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const verifyMail = async (e) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(
        "http://localhost:8080/api/authentication/verify-mail",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handlePassword = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/api/authentication/update-password",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
